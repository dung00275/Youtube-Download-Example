...
//        let escapedPath = CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, originalString, charactersToLeaveUnescaped, legalURLCharactersToBeEscaped, encoding)
        ...
        let charset = NSCharacterSet.URLUserAllowedCharacterSet()
        let path = self.stringByAddingPercentEncodingWithAllowedCharacters(charset) ?? ""
        return path //escapedPath as NSString as String
    }
